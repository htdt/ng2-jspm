...
export function main() {
  ...
  function createMetric(perfLogs, microIterations = 0) {
    commandLog = [];
    var bindings = [PerflogMetric.BINDINGS, bind(PerflogMetric.SET_TIMEOUT).toValue((fn, millis) => {
    ...
    describe('aggregation', () => {
      ...
      describe('microIterations', () => {
        it('should not report scriptMicroAvg if microIterations = 0', (done) => {
          aggregate([eventFactory.start('script', 0), eventFactory.end('script', 5)], 0).then((data) => {
            expect(isPresent(data['scriptMicroAvg'])).toBe(false);
            done();
          });
        });
        it('should report scriptMicroAvg', (done) => {
          aggregate([eventFactory.start('script', 0), eventFactory.end('script', 5)], 4).then((data) => {
            expect(data['script']).toBe(5);
